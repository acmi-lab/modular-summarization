...
@Model.register("conditioned_pointer_generator")
class Seq2Seq(Model):
    def __init__(self, vocab, hidden_size=256, token_emb_size=128, num_encoder_layers=1, num_decoder_layers=1, min_decode_length=0, max_decode_length=9999, use_copy_mech=True, conditioning_emb_size=32):
...
#             print(torch.sum(pgen), torch.sum(pgen*mask_labels), torch.sum(mask_labels).detach())
    ...
    def decode_onestep(self, past_outp_input, section_label, past_state_tuple, past_context_vector, inp_enc_seq, inp_with_oovs, input_pad_mask, max_oovs):
        '''run one step of decoder. past_outp_input is batchsizex1
...
#         output_ids = self.greedy_decode(**model_input, min_length=self.min_decode_length, max_length=self.max_decode_length)
        ...
        for _id in output_ids:
            if _id<self.vocab_size:
                output_words.append(self.vocab.get_token_from_index(_id, namespace="tokens"))
            else:
