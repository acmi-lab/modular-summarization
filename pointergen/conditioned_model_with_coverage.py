...
class DualEmbedder(nn.Module):
    def __init__(
            self, token_vocab_size, token_emb_size, conditioning_vocab_size, conditioning_emb_size):
        super(DualEmbedder, self).__init__()
        self.token_embedder = nn.Embedding(num_embeddings=token_vocab_size, embedding_dim=token_emb_size)
        self.conditioning_embedder = nn.Embedding(num_embeddings=conditioning_vocab_size, embedding_dim=conditioning_emb_size)

    def forward(
            self, token_batch, condition_batch):
        '''token_batch = batchXseqlen
            condition_batch = batch'''
        embedded_token_seq = self.token_embedder(token_batch)
        embedded_conditioning_seq = self.conditioning_embedder(condition_batch.unsqueeze(1))
        
        seq_len = token_batch.shape[1]
        embedded_conditioning_seq_repeated = embedded_conditioning_seq.repeat(1,seq_len,1)
        return torch.cat([embedded_token_seq, embedded_conditioning_seq_repeated], axis=-1)
        
        
@Model.register("conditioned_pointer_generator_withcoverage")
class Seq2Seq(Model):
    def __init__(self, vocab, hidden_size=256, token_emb_size=128, num_encoder_layers=1, num_decoder_layers=1, min_decode_length=0, max_decode_length=9999, use_copy_mech=True, initial_precoverage_paramfile=None, coverage_coef=1.0, conditioning_emb_size=32):
        super().__init__(vocab)
        # self.vocab=vocab

        ## vocab related setup begins
        self.vocab_size=vocab.get_vocab_size(namespace="tokens")
        self.PAD_ID = vocab.get_token_index(vocab._padding_token, namespace="tokens")
        self.OOV_ID = vocab.get_token_index(vocab._oov_token, namespace="tokens")
        self.START_ID = vocab.get_token_index(START_SYMBOL, namespace="tokens")
        self.END_ID = vocab.get_token_index(END_SYMBOL, namespace="tokens")
        

        self.conditioning_vocab_size = vocab.get_vocab_size(namespace="section_labels")
        ## vocab related setup ends
        
        

        self.token_emb_size = token_emb_size
        self.conditioning_emb_size = conditioning_emb_size
        ...
        self.output_embedder = DualEmbedder(self.vocab_size, self.token_emb_size, self.conditioning_vocab_size, self.conditioning_emb_size)
                
        self.encoder_rnn = torch.nn.LSTM(input_size=self.token_emb_size+self.conditioning_emb_size, hidden_size=self.hidden_size, num_layers=self.num_encoder_layers, batch_first=True, bidirectional=True)
        ...
        if self.use_copy_mech:
            self.copymech=CopyMechanism(2*self.hidden_size, self.hidden_size, self.token_emb_size+self.conditioning_emb_size)
        
        self.decoder_rnn=torch.nn.LSTM(input_size=self.token_emb_size+self.conditioning_emb_size, hidden_size=self.hidden_size, num_layers=self.num_decoder_layers, batch_first=False, bidirectional=False)
        
        self.statenctx_to_prefinal = nn.Linear(3*hidden_size, hidden_size, bias=True)
        self.project_to_decoder_input = nn.Linear(self.token_emb_size+self.conditioning_emb_size+2*hidden_size, self.token_emb_size+self.conditioning_emb_size, bias=True)
    ...
    def forward(self, source_tokens, target_tokens, section_labels, meta=None, only_predict_probs=False, return_pgen=False):
        ...
        inp_enc_seq, (last_h_value, last_c_value) = self.encode(inp_with_unks, section_labels)
        ...
        output_embedded = self.output_embedder(feed_tensor, section_labels)
    ...
    def encode(self, inp, section_labels):
        '''Get the encoding of input'''
        batch_size = inp.size(0)
        inp_seq_len = inp.size(1)
        inp_embedded = self.output_embedder(inp, section_labels)
        inp_encoded = self.encoder_rnn(inp_embedded)
    ...
    def decode_onestep(self, past_outp_input, section_label, past_state_tuple, past_context_vector, inp_enc_seq, inp_with_oovs, input_pad_mask, max_oovs, past_coverage_vector):
        '''run one step of decoder. outp_input is batchsizex1
        section_label is batchsize
        past_context_vector is batchsizeX1Xtwice_of_hiddensize
        past_coverage_vector is batchsizeXenc_len'''
        outp_embedded = self.output_embedder(past_outp_input, section_label)
    ...
    @overrides
    def forward_on_instance(self, instance: SyncedFieldsInstance, decode_strategy=None) -> Dict[str, str]:
        """
    ...
    def beam_search_decode(self, source_tokens, section_labels, target_tokens=None, meta=None, beam_width=4, min_length=35, max_length=120):
        inp_with_unks = source_tokens["ids_with_unks"]
        inp_with_oovs = source_tokens["ids_with_oovs"]
        max_oovs = int(torch.max(source_tokens["num_oovs"]))
        input_pad_mask=torch.where(inp_with_unks!=self.PAD_ID, self.true_rep, self.false_rep)
        inp_enc_seq, (intial_h_value, intial_c_value) = self.encode(inp_with_unks, section_labels)
        ...
        while counter<max_length and len(finished_hypotheses)<beam_width:
            ...
            for hyp in hypotheses:
                ...
                with torch.no_grad():
                    new_probs, new_dec_state, new_context_vector, attn_weights = self.decode_onestep( torch.tensor([[in_tok]]).cuda(), section_labels, old_dec_state, past_context_vector, inp_enc_seq, inp_with_oovs, input_pad_mask, max_oovs, past_coverage_vector)
    ...
    def greedy_decode(self, source_tokens, section_labels, target_tokens=None, meta=None, min_length=35, max_length=120):
        inp_with_unks = source_tokens["ids_with_unks"]
        inp_with_oovs = source_tokens["ids_with_oovs"]
        max_oovs = int(torch.max(source_tokens["num_oovs"]))
        input_pad_mask=torch.where(inp_with_unks!=self.PAD_ID, self.true_rep, self.false_rep)
        ...
        while counter<max_length:
            ...
            past_context_vector=hyp["past_context_vector"]
            old_logprobs=hyp["logprobs"]
            new_probs, new_dec_state, new_context_vector = self.decode_onestep( torch.tensor([[in_tok]]).cuda(), section_labels, old_dec_state, past_context_vector, inp_enc_seq, inp_with_oovs, input_pad_mask, max_oovs)
